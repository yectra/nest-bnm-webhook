...
| `adversaryFilter` | Prompt-injection guard, provided by the reusable [`adversary-guard` module](./adversary-guard.md): regex patterns → semantic match against the `AdversarialInputs` Cosmos DB vector container of known-attack exemplars → borderline review by a low-cost open-source model. Fails open so real customers are never locked out; confirmed attacks are learned back into the vector container. Flagged messages never reach the planner, retrieval, or synthesis LLMs — they jump straight to the PII filter with a safe canned reply. All crew prompts additionally instruct the models to treat customer text as data, not instructions. |
...
| `ADVERSARY_GUARD_*` | see [adversary-guard.md](./adversary-guard.md) | Vector container, thresholds, and the open-source review model for the adversary filter. |
